...
{showWhatsApp && (
  <div className="bg-green-500 text-white py-2 px-4 md:px-6 flex flex-col md:flex-row justify-between items-center">
    <span className="text-sm md:text-base font-medium mb-2 md:mb-0">
      Supplier of All Types Of Papads in India.
    </span>
    <a
      href="https://api.whatsapp.com/send?phone=9725399996&text=I'm%20interested%20in%20your%20products"
      target="_blank"
          ...
          <nav className="hidden md:flex items-center space-x-8 md:space-x-12">
            {navItems.map((item) => (
              <Link 
                key={item.name}
                to={item.path}
                className="flex-shrink-0 inline-flex items-center justify-center gap-10 w-auto h-full text-[#1E1E1E] text-center font-poppins  font-bold leading-normal tracking-[0.84px] hover:text-yellow-700 transition-colors"
              >
                {item.name}
              </Link>
            ))}
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="font-poppins font-bold text-[#1E1E1E] hover:text-yellow-700 transition-colors flex items-center"
              >
                INQUIRY <ChevronDown className="h-4 w-4 ml-1" />
              </button>
              {showChannelPartner && (
                <div className="absolute top-full right-0 mt-4 w-48 bg-white rounded-md shadow-lg z-10 border border-yellow-400 animate-fadeIn font-poppins">
                  <Link 
